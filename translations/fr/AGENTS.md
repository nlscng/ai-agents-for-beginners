...
4. **Set up environment variables:**
   ```bash
   cp .env.example .env
   # Modifiez le fichier .env avec vos clés API et vos points de terminaison
   ```
...
2. **Test notebook execution:**
   ```bash
   # Convertir le notebook en script et l'exécuter (imports des tests)
